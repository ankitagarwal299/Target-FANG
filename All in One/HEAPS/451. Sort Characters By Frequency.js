...
var frequencySort = function (s) {
  ...
  for (let [char, value] of countMap.entries()) {
    maxHeap.add(char);
  }

  let result = "";
  while (maxHeap.size() > 0) {
    let char = maxHeap.poll();

    while (countMap.get(char) > 0) {//trick- get the char count and repeat char adition for result
      result += char
      countMap.set(char, countMap.get(char) - 1);//decrement by -1
    }
  }
  return result;
};
